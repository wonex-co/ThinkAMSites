...
using ThinkAM.Sites.Web.Startup;
using AngleSharp.Html.Dom;
using AngleSharp.Html.Parser;
using Microsoft.AspNetCore.Hosting;
using Shouldly;

namespace ThinkAM.Sites.Web.Tests
{
    public abstract class SitesWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static SitesWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(SitesWebMvcModule).Assembly.FullName);
        }
        ...
        {
            using (var context = IocManager.Resolve<SitesDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected async Task UsingDbContextAsync(Func<SitesDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<SitesDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<SitesDbContext, Task<T>> func)
        {
            T result;
